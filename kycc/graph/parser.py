...
from kycc.graph.models import TxNode, UTXONode, ScriptType
from kycc.script_utils import classify_script, is_rbf, locktime_type


def parse_tx(raw: dict[str, Any]) -> TxNode:
    """
    Parse output of getrawtransaction(txid, verbose=2) into a TxNode.
    verbose=2 resolves prevouts inline — no second RPC call needed per input.
    """
    vin  = raw.get("vin", [])
    vout = raw.get("vout", [])
    ...
    return TxNode(
        ...
        locktime_type= locktime_type(raw.get("locktime", 0)),
...
def _parse_inputs(vin: list[dict], is_coinbase: bool) -> list[UTXONode]:
    if is_coinbase:
        return [UTXONode(
            txid="0" * 64, vout=0xFFFFFFFF,
            value_sats=0, script_pubkey_hex="",
            script_type="coinbase", address=None,
            is_spent=True, spending_txid=None,
        )]
    ...
    for inp in vin:
        ...
        spk_hex = spk.get("hex", "")
        sats    = round(prevout.get("value", 0) * 100_000_000)

        result.append(UTXONode(
            txid              = inp["txid"],
            vout              = inp["vout"],
            value_sats        = sats,
            ...
            is_spent          = True,
            spending_txid     = None,   # filled by resolver later
        ))
    return result
...
def _parse_outputs(txid: str, vout: list[dict]) -> list[UTXONode]:
    ...
    for out in vout:
        ...
        spk_hex = spk.get("hex", "")
        sats    = round(out.get("value", 0) * 100_000_000)
        ...
        result.append(UTXONode(
            ...
            script_type       = classify_script(spk_hex),
            address           = spk.get("address"),
            is_spent          = False,      # unknown at parse time
            spending_txid     = None,
        ))
    return result