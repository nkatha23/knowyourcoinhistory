...
class LabelStore:
    ...
    def upsert(self, label: Label) -> None:
        ...
        with self._conn() as conn:
            ...
            """, (
                label.wallet_id, label.ref_type, label.ref,
                label.label, label.origin,
                int(label.spendable) if label.spendable is not None else None,
                now, now,
            ))

    def get(self, ref_type: RefType, ref: str,
            wallet_id: str = "default") -> Optional[Label]:
        with self._conn() as conn:
            row = conn.execute("""
                SELECT * FROM labels
                WHERE wallet_id=? AND ref_type=? AND ref=?
            """, (wallet_id, ref_type, ref)).fetchone()
        return _row_to_label(row) if row else None

    def delete(self, ref_type: RefType, ref: str,
               wallet_id: str = "default") -> None:
        with self._conn() as conn:
            conn.execute("""
                DELETE FROM labels
                WHERE wallet_id=? AND ref_type=? AND ref=?
    ...
    def list(self, wallet_id: str = "default",
             ref_type: Optional[RefType] = None) -> list[Label]:
        with self._conn() as conn:
            if ref_type:
                rows = conn.execute("""
                    SELECT * FROM labels
    ...
    def hydrate_utxo(self, txid: str, vout: int,
                     wallet_id: str = "default") -> Optional[str]:
