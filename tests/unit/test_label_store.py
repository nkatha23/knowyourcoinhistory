import tempfile
from kycc.labels.store import LabelStore, Label
...
def test_upsert_and_get():
    store, _ = make_store()
    store.upsert(Label(
        wallet_id="default", ref_type="tx",
        ref="a"*64, label="Exchange withdrawal"
    ))
...
def test_upsert_updates_existing():
    ...
    store.upsert(Label(wallet_id="default", ref_type="tx", ref="a"*64, label="updated"))
...
def test_list_all():
    store, _ = make_store()
    store.upsert(Label(wallet_id="default", ref_type="tx",   ref="a"*64, label="tx label"))
...
def test_wallet_id_isolation():
    store, _ = make_store()
    store.upsert(Label(wallet_id="wallet_a", ref_type="tx", ref="a"*64, label="from a"))
    store.upsert(Label(wallet_id="wallet_b", ref_type="tx", ref="a"*64, label="from b"))
...
def test_hydrate_tx():
    store, _ = make_store()
    store.upsert(Label(wallet_id="default", ref_type="tx", ref="a"*64, label="hydrated"))
...
def test_hydrate_utxo():
    store, _ = make_store()
    store.upsert(Label(wallet_id="default", ref_type="utxo", ref="a"*64+":0", label="dust"))
