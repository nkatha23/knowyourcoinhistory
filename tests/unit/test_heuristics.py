...
def make_tx(
    ...
    outputs = outputs or [make_utxo(sats=99_000, address="bc1qout", is_spent=False)]
    return TxNode(
        txid="a" * 64,
...
def test_change_inference_triggers():
    ...
    chg = make_utxo(
        script_type="p2wpkh", sats=39_000, address="bc1qchg", is_spent=False
    )
...
def test_engine_empty_result_for_clean_tx():
    ...
    tx = make_tx(
        ...
        outputs=[
            make_utxo(
                sats=99_000, address="bc1qout", script_type="p2wpkh", is_spent=False
            )
        ],
        is_rbf=False,
        locktime=0,
