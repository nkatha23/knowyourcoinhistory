...
def test_create_session(client):
    res = client.post(
        "/api/session", json={"root_txid": MOCK_TXID, "wallet_ids": ["default"]}
    )
    data = res.get_json()
...
def test_create_session_invalid_txid(client):
    res = client.post(
        "/api/session", json={"root_txid": "tooshort", "wallet_ids": ["default"]}
    )
...
def test_get_session_roundtrip(client):
    create = client.post(
        "/api/session",
        json={"root_txid": MOCK_TXID, "wallet_ids": ["wallet_a", "wallet_b"]},
    ).get_json()
